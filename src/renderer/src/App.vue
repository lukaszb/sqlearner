...
import SessionTransfer from '@/renderer/src/components/SessionTransfer.vue'
...
watch(() => JSON.stringify({ queryTabs: store.queryTabs, activeQueryTabId: store.activeQueryTabId,
  selectedTable: store.selectedTable }), (snapshot) => {
  if (!store.workspaceReady || !store.activeSessionId || !window.sqlearner?.saveSessionWorkspace) return
  void window.sqlearner.saveSessionWorkspace(store.activeSessionId, JSON.parse(snapshot))
    .catch((failure: unknown) => { store.error = failure instanceof Error ? failure.message : 'Failed to save query workspace' })
}, { flush: 'sync' })
...
<template>
  <div class="flex h-screen flex-col">
    <SessionTransfer :session-id="store.activeSessionId" />
    <div v-if="lessonsStore.error" role="alert" class="bg-red-50 px-6 py-3 text-sm text-red-700">{{ lessonsStore.error }}</div>
    <main class="flex min-h-0 flex-1 bg-panel text-ink" data-testid="app-shell">
