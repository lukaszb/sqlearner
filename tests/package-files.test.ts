...
describe('electron-builder file globs', () => {
  ...
  it('builds standalone Windows and macOS release artifacts', () => {
    ...
    expect(packageJson.scripts['build:mac']).toContain('mac.identity=-')
    expect(packageJson.scripts['build:mac']).toContain('mac.hardenedRuntime=false')
    expect(packageJson.scripts['build:mac:release']).toContain('mac.forceCodeSigning=true')
    expect(packageJson.scripts['build:mac:release']).toContain('mac.notarize=true')
    expect(packageJson.scripts['build:release']).toContain('mac.forceCodeSigning=true')
    expect(packageJson.scripts['build:release']).toContain('mac.notarize=true')
  })
})
